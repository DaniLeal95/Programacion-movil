...
public class MainActivity extends ListActivity implements View.OnClickListener,AdapterView.OnItemLongClickListener, PopupMenu.OnMenuItemClickListener {
    ...
    public Utilidades u= new Utilidades(this);
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(bundle !=null){
            Jugador j= (Jugador) bundle.getSerializable("Jugador");
            ((ListadoJugadores) getApplication()).setJugadores(j);
            jugadores=((ListadoJugadores) getApplication()).getJugadores();       ((ListadoJugadores) getApplication()).elimarimagen(j.getImg());
            u.escribirJugador((((ListadoJugadores) getApplication())).getJugadores());
        }
        ...
        if(((ListadoJugadores) getApplication()).getJugadores()!=null && ((ListadoJugadores) getApplication()).getJugadores().size()>0)
            jugadores = ((ListadoJugadores) getApplication()).getJugadores();
        else{
            jugadores = u.getJugadores("jugadores.dat");
            ((ListadoJugadores) getApplication()).setJugadores(jugadores);
